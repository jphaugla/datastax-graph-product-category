...
class JODIE(nn.Module):
    def __init__(self, num_features, num_users, num_items, embedding_dimensions, model_name):
...
def save_model(model, optimizer, network, modelname, epoch, user_embeddings, item_embeddings, train_end_idx, total_loss, user_embeddings_time_series=None, item_embeddings_time_series=None, path=PATH):
    ...
    directory = os.path.join(path, 'saved_models/%s' % network)
    if not os.path.exists(directory):
        os.makedirs(directory)


    filename = os.path.join(directory, "checkpoint.%s.ep%d.tp%.1f.pth.tar" % (modelname, epoch, 1))
    ...
    session.execute(
        """
        INSERT INTO prodcat.model (version, epoch, model, loss)
        VALUES (%s, %s, %s, %s)
        """,
        ("0.1", epoch, buffer.getbuffer(), total_loss)
    )
...
def load_model(model, optimizer, modelname, network, epoch):
    cluster = Cluster()
    session = cluster.connect()

    results = session.execute(
        """
        SELECT * FROM prodcat.model WHERE version='0.1' AND epoch=%s
        """,
        (epoch, )
    )
    checkpoint = None
    ...
    if len(results.current_rows) > 0:
        row = results.current_rows[0]
        checkpoint = torch.load(io.BytesIO(row.model))
    else:
    ...
    item_embeddings = Variable(torch.from_numpy(checkpoint['item_embeddings']))
    try:
    ...
    try:
        user_embeddings_time_series = Variable(torch.from_numpy(checkpoint['user_embeddings_time_series']))
        item_embeddings_time_series = Variable(torch.from_numpy(checkpoint['item_embeddings_time_series']))
    except:
    ...
    return [model, optimizer, user_embeddings, item_embeddings, user_embeddings_time_series, item_embeddings_time_series, train_end_idx, checkpoint['epoch']]

def set_embeddings(user_embeddings, item_embeddings, user_embeddings_time_series, item_embeddings_time_series, interaction):
    try:
        embedding_dim = user_embeddings_time_series.size(1)
    except:
        embedding_dim = user_embeddings_time_series.shape[1]

    user_embeddings[interaction.user_sequence_id, :embedding_dim] = user_embeddings_time_series[0]
    item_embeddings[interaction.product_sequence_id, :embedding_dim] = item_embeddings_time_series[0]
