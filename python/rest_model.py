...
def process_model(interaction):
    # interactions = parse_interaction_data(results, start_time)
    interaction_details = calc_interaction_details([interaction])
    model = initialize_model(interaction_details)

    predictions = evaluate_model(interaction, interaction_details, model, 149)
...
def evaluate_model(interaction, interaction_details, model, epoch):
    ...
    for i in range(0, len(probabilities[0])):
        ...
        if abs(prob) > probabilities[0][predicted_state_change]:
