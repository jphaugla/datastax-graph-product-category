...
# Sets the training, validation and testing markers.
# training start is 80% of the data set
# validation is 10% of the data set
# testing is 10% of the data set
# These are also used for tbatch calculations on where to stop and start on each sequence
...
# Grab the final timestamp and subtract it from the first timestamp sequence
# to calculate the total range of timestamps
# Evenly divide this range by 500 to create N number of tbatches
                ...
                if timestamp - tbatch_start_time > tbatch_timespan:
                    ...
                    with trange(len(lib.current_tbatches_user)) as progress_bar3:
                        for i in progress_bar3:
                            ...
                            tbatch_userids = torch.LongTensor(lib.current_tbatches_user[i]) # Recall "lib.current_tbatches_user[i]" has unique elements
                            tbatch_itemids = torch.LongTensor(lib.current_tbatches_item[i]) # Recall "lib.current_tbatches_item[i]" has unique elements
                            tbatch_interactionids = torch.LongTensor(lib.current_tbatches_interactionids[i])
                            feature_tensor = Variable(torch.Tensor(lib.current_tbatches_feature[i])) # Recall "lib.current_tbatches_feature[i]" is list of list, so "feature_tensor" is a 2-d tensor
                            user_timediffs_tensor = Variable(torch.Tensor(lib.current_tbatches_user_timediffs[i])).unsqueeze(1)
                            item_timediffs_tensor = Variable(torch.Tensor(lib.current_tbatches_item_timediffs[i])).unsqueeze(1)
